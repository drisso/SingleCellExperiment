...
altExperimentNames(sce)
```

This data is easily retrieved with the `altExperiment()` method in functions that need to use spike-in information.
...
```{r}
altRowData(sce)$concentration <- runif(nrow(altExperiment(sce)))
altRowData(sce)
rowData(sce)
```
